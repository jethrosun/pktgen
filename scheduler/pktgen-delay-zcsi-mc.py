...
def restart_pktgen(handle, port, pcie0, pcie1, pcie2, pcie3):
    if handle:
        handle.kill()
        handle.wait()
...
def measure_delay(q, pgen_server, pgen_port, server, out):
    ...
    start_zcsi = "/opt/e2d2/scripts/start-zcsi-mc.sh start 1 %d"
    stop_zcsi = "/opt/e2d2/scripts/start-zcsi-mc.sh stop"
